...
| **"Two-owner rule" is slightly aspirational**                       | Docs say only `TaskMaster` (writes) and `StatusNotifier` (reads) touch `task_status`, but `TaskStatusTracker` also touches it directly (shared by both).   | Harmless — the recorder *is* the shared persistence utility.                                               | If a third caller appears.                              |
