...
| **`TaskStatusTracker::create_run` is check-then-act**             | `submit_inner` reads the latest run, then inserts. Two concurrent submitters can both pick the same run number.                                                   | The `(envelope_id, run)` unique index arbitrates: the loser's insert fails and is surfaced as a refusal (`Ok(false)`), so correctness does not depend on the read. | If the refusal path ever gets logged as an unexpected error, or a second user is added.        |

### Operational

| Issue                                                 | Effect                                                                                                                              | Why tolerated                                        | Revisit trigger                                                             |
| ----------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- | --------------------------------------------------------------------------- |
| **No retention / vacuum policy on `task_status`**     | The table grows forever. Note the run dimension means a reran task keeps *all* its rows, so growth is per-run rather than per-task. | Tiny table, single user.                             | Row count becomes non-trivial, or query latency degrades.                   |
...
| **"Two-owner rule" is slightly aspirational**                       | Docs say only `TaskMaster` (writes) and `StatusListener` (reads) touch `task_status`, but `TaskStatusTracker` also touches it directly (shared by both). | Harmless — the recorder *is* the shared persistence utility.                                               | If a third caller appears.                              |
...
| `TaskStatusTracker::record` was SELECT-then-INSERT, not an atomic upsert                   | Replaced by `create_run` (insert, unique-violation → `Ok(false)`) + `update_run` (targeted update). Duplicate submission is now *refused*, not a DB error.     |
