...
| **`query_status(envelope_id)` is not user-scoped**                | `envelope_id` embeds `user_id`, so it's *implicitly* scoped, but the predicate isn't enforced.                                                                    | Internal-only call, used by `begin_attempt`. The public queries *are* explicitly user-scoped.                                                         | If it ever becomes reachable from a request handler.                                           |
