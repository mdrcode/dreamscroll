...
| **"Two-owner rule" is slightly aspirational**   | Docs say only `TaskMaster` (writes) and `StatusListener` (reads) touch `task_run_status`, but `TaskStatusTracker` also touches it directly (shared by both). | Harmless — the tracker is the shared persistence utility.                                                  | If a third caller appears.                  |
