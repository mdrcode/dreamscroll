...
#[derive(Clone)]
pub struct UrlMaker {
    config: UrlMakerConfig,
}

impl UrlMaker {
    pub fn new(config: UrlMakerConfig) -> Self {
        Self { config }
    }
}
...
impl UrlMaker {
    ...
    pub fn make_local_url(&self, id: &StorageIdentity) -> String {
        self.config
            .local_url_prefix
            .as_ref()
            .map(|prefix| format!("{}/{}", prefix, id.provider_id))
            .unwrap_or(id.provider_id.clone())
    }

    pub fn make_gcloud_url(&self, id: &StorageIdentity) -> String {
        // For emulator, return emulator URL
        if let Some(ref endpoint) = self.config.gcloud_emulator_endpoint {
            format!(
